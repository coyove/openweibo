...
func (m *Image) distort(amplude float64, period float64) {
	...
	// 0.04 - 0.1
	period = rand.Float64()*0.06 + 0.04
	dx := 2.0 * math.Pi / period
	offsetx, offsety := rand.Float64()*dx, rand.Float64()*dx
	// offsetx, offsety = 0, 0
	for x := 0; x < w; x++ {
		for y := 0; y < h; y++ {
			xo := amplude * (math.Sin((offsetx + float64(y)) * period))
			yo := amplude * (math.Sin((offsety + float64(x)) * period))
