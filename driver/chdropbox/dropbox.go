...
func (s *Storage) newReq(method, path string, r io.Reader) *http.Request {
...
func (s *Storage) calcPath(k string) string {
	k = sanitize(k)
	h := sha1.Sum([]byte(k))
...
func (s *Storage) Put(k string, v []byte) error {
	...
	req := s.newReq("POST", "https://content.dropboxapi.com/2/files/upload", bytes.NewReader(v))
...
func (s *Storage) Get(k string) ([]byte, error) {
	...
	req := s.newReq("POST", "https://content.dropboxapi.com/2/files/download", nil)
	req.Header.Add("Content-Type", "application/octet-stream")
...
func (s *Storage) Delete(k string) error {
	...
	req := s.newReq("POST", "https://api.dropboxapi.com/2/files/delete_v2", strings.NewReader(
...
func (s *Storage) Stat() driver.Stat {
	...
	start := time.Now()
	req := s.newReq("POST", "https://api.dropboxapi.com/2/users/get_space_usage", nil)
	resp, err := s.client.Do(req)
	if err != nil {
